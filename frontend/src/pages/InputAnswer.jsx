...
function MCQGrid({ data, setData }) {
  const questions = [1, 2, 3, 4]; // Ví dụ 4 câu
...
function ShortGrid({ data, setData }) {
  const questions = Array.from({ length: 6 }, (_, i) => i + 1); // 6 câu

  // Hàm kiểm tra định dạng hợp lệ
  const isValidAnswer = (value) => {
    // Cho phép: số (1-4 chữ số), dấu -, dấu ., kết hợp
...
export default function InputAnswer() {
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <h3>Phần 2 – Đúng/Sai theo ý</h3>
      <TFGrid data={part2} setData={setPart2} />

      <h3>Phần 3 – Nhập đáp án số</h3>
